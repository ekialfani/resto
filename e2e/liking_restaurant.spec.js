...
Scenario('liking one restaurant', ({ I }) => {
  I.see('No restaurant to like yet', '.empty-message');
  I.amOnPage('/');
  I.waitForElement('.restaurant-item .title a', 20);
...
Scenario('disliking one restaurant', ({ I }) => {
  ...
  I.waitForElement('.restaurant-item .title a', 20);
  I.seeElement('.restaurant-item .title a', 20);

  const restaurantItem = locate('.restaurant-item .title a');
  I.click(restaurantItem);

  I.waitForElement('[aria-label="like this restaurant"]', 20);
  ...
  I.amOnPage('/#/favorite');
  I.waitForElement('.empty-message', 20);
  I.see('No restaurant to like yet', '.empty-message');
});
